...
import { db, Recorrido360Config } from '@/db/database'
...
const Recorrido360Page: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(null)
      setSuccess(false)
      if (recorridoUrl && !recorridoUrl.startsWith('http')) {
    ...
    <Box>
      <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Stack direction="row" spacing={2} alignItems="center">
          <ThreeSixtyIcon sx={{ fontSize: 40, color: '#334155' }} />
          <Box>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#1a1a1a' }}>
              Recorrido 360°
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Visualiza la obra con recorridos virtuales
            </Typography>
          </Box>
        </Stack>
        {canEdit && !editMode && recorridoUrl && (
          <Tooltip title="Editar enlace del recorrido">
            <IconButton 
      ...
      {success && <Alert severity="success" sx={{ mb: 3 }}>Recorrido guardado correctamente</Alert>}

      {canEdit && (editMode || !recorridoUrl) && (
        <Card sx={{ mb: 3, borderRadius: 3, boxShadow: 3 }}>
          <CardContent>
            <Typography variant="h6" sx={{ mb: 2, fontWeight: 600 }}>
              Configurar Enlace del Recorrido 360°
            </Typography>
            <Stack spacing={2}>
              <TextField
                fullWidth
                label="URL del Recorrido (Matterport, Kuula, etc.)"
                placeholder="https://my.matterport.com/show/?m=..."
                value={recorridoUrl}
                onChange={(e) => setRecorridoUrl(e.target.value)}
              ...
              <Stack direction="row" spacing={2}>
                <Button variant="contained" startIcon={saving ? <CircularProgress size={20} /> : <SaveIcon />}
                  onClick={handleSave} disabled={saving || !recorridoUrl.trim()}
      ...
      {!recorridoUrl ? (
        <Alert severity="warning" sx={{ borderRadius: 3 }}>
          <Typography variant="body1" sx={{ fontWeight: 600, mb: 1 }}>
            No se encontró el enlace del Recorrido 360°
          </Typography>
          <Typography variant="body2">
            {canEdit ? 'Por favor, configura el enlace del tour virtual arriba.' : 'Contacta al administrador para configurar el recorrido.'}
          </Typography>
        </Alert>
      ) : (
        <Paper elevation={3} sx={{ borderRadius: 3, overflow: 'hidden', height: 'calc(100vh - 300px)', minHeight: 600 }}>
          <iframe src={recorridoUrl} style={{ width: '100%', height: '100%', border: 0 }} title="Recorrido 360°" allowFullScreen />
        </Paper>
      )}
    </Box>
  )
...
export default Recorrido360Page
